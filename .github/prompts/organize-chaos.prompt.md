...
Execute a **complete, end-to-end workflow** to transform all uncommitted local changes into properly documented, tracked, and merged development work. This is a fully automated operation—proceed through all phases without waiting for user confirmation between steps unless a critical decision or error requires intervention.
...
|---|-------|--------|-----|--------|
| 1 | #[num] [PREFIX-####] Title | `feature/...` | #[pr] | ✅ Merged |
| 2 | #[num] [PREFIX-####] Title | `bugfix/...` | #[pr] | ✅ Merged |

---

### Issues Created

1. **#[num]** — [[PREFIX-####] Title](link)
2. **#[num]** — [[PREFIX-####] Title](link)
...
**Your chaos has been transformed into clean, traceable development history! 🎉**
```
...
- `${input:scope:Optional - specific files or directories to organize}` — If provided, limit analysis to these paths. If empty, analyze ALL uncommitted changes.
...
- ✅ Analyze ALL uncommitted changes thoroughly
- ✅ Create separate issues for each logical group (never combine unrelated changes)
- ✅ Use proper issue prefixes (FR-, BG-, HF-, TC-, SC-, PF-, DC-, IN-)
- ✅ Query existing issues to determine next available ID
...
- ❌ Combine unrelated changes into a single issue/PR
...
- ⚠️ If CI fails and cannot be fixed: Report and continue with other groups
- ⚠️ If merge conflicts occur: Report and request user intervention
- ⚠️ If permissions denied: Report and halt that specific operation
